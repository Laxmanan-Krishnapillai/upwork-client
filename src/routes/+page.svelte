...
<header>
	<div class="con">
		...
		<div col="6" mcol="12">
			<div class="wrap left">
				<h1>
					Designing <i>impossible-to-ignore</i> brands and competitive digital solutions for
					design-driven companies where <i>attention to detail</i> matters.
				</h1>
				<h2>
					Brand Strategy, <i>Brand Identity</i>, Creative Development, <i>Creative Direction</i>,
					Digital Design, <i>Digital Engineering</i>, Experience Design.
				</h2>
	...
	<div class="con">
		...
		{#each works as { name, url, media, client, services, date }}
			<div class="case">
				<div id="top">
					<h2 col="6">{name}</h2>
					<!-- reference: clientName will be here -->
					<h3 col="4" op="40" mcol="6">{client.industry}</h3>
					<!-- reference: clientIndustry -->
					<p col="2" op="40" hide="600">{client.location}</p>
					<!-- reference: clientLocation -->
				</div>
				<div class="marquee">
					<div class="track">
				...
				<div id="bot">
					<p col="10" mcol="12" op="50">{services.map((service) => service.name).join(', ')}</p>
					<!-- reference: services will be here -->
					<p col="2" mcol="12" mfirst op="50">{date}</p>
					<!-- Launch date will be here -->
				</div>
			</div>
		{/each}
	</div>
